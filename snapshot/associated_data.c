...
void remove_mm_data(struct mm_data *data) { list_del(&data->l); }

/*
 * from mm.h
 */
void clear_snapshot(struct mm_data *data) { data->ss.status &= ~SNAPSHOT_MADE; }

void set_had_snapshot(struct mm_data *data) { data->ss.status |= SNAPSHOT_HAD; }

void set_snapshot(struct mm_data *data) { data->ss.status |= SNAPSHOT_MADE; }
...
void remove_files_data(struct files_data *data) { list_del(&data->l); }
