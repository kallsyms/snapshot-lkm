...
//#define DEBUG
#ifdef DEBUG
#define dbg_printk(format, arg...) printk(pr_fmt(format), ##arg);
#else
...
void do_recover_page(struct snapshot_page *sp) {
  ...
  dbg_printk("[WEN] found reserved page: 0x%08lx page_base: 0x%08lx page_prot: "
             "0x%08lx\n",
...
void do_recover_none_pte(struct snapshot_page *sp) {
  ...
  dbg_printk(
      "[WEN] found none_pte refreshed page_base: 0x%08lx page_prot: 0x%08lx\n",
  ...
  page = pte_page(*pte);
  dbg_printk("[WEN] re-checking addr: 0x%08lx PTE: 0x%08lx Page: 0x%08lx
...
void make_snapshot_page(struct vm_area_struct *vma, unsigned long addr) {
  ...
  dbg_printk("[WEN] making snapshot: 0x%08lx PTE: 0x%08lx Page: 0x%08lx "
             "PageAnon: %d\n",
...
void clean_context(struct mm_data *mdata) {
  ...
  if (mdata->ss.ucontext != NULL)
    kfree(mdata->ss.ucontext);
 ...
 * hooks
 */
static long return_0_stub_func(void) { return 0; }
...
int wp_page_hook(struct kprobe *p, struct pt_regs *regs) {
  ...
  if (is_snapshot_page_private(ss_page)) {
    ...
    // printk("[WEN] page fault! process: %s addr: 0x%08lx ptep: 0x%08lx pte:
    // 0x%08lx\n",
